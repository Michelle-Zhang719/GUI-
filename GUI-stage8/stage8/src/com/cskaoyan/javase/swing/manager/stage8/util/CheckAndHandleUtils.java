package com.cskaoyan.javase.swing.manager.stage8.util;
...
import com.cskaoyan.javase.swing.manager.stage8.model.NameLimit;
...
public class CheckAndHandleUtils {
    ...
    private final static StudentController studentController = new StudentControllerImpl();
    // 成员变量properties
    private static Properties properties;
    // 使用静态代码块给properties成员变量赋值, 并加载config配置文件
    static {
        try {
            properties = new Properties();
            properties.load(new FileInputStream("config.properties"));
            // 当静态代码执行完之后,已经加载完config,可以使用getProperty(String key)方法获取相关的value值
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
